"""
Creates bounding boxes from the clustered lowest points together with a slope analysis for each cluster.
...
def get_bounding_boxes(pointcloud_array, is_3d=False):
    ...
    for label, cluster in zip(unique_labels, expanded_clusters):
        ...
        if is_3d:
            ...
            min_z = cluster_pointcloud[:, 2].max() - STANDARD_DEPTH # closer to the left side of the vehicle = higher z
